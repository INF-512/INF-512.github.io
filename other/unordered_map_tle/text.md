...
$2.$ 当元素数量达到上限时，`unordered_map` 会新建一个大小至少是原来两倍且是质数的索引表，然后把原来的索引表拷贝到新的索引表，最后删除原来的索引表。
...
`unordered_map` 被卡成 $O(n^2)$ 这种情况是不应该出现，因为按照常规思路，`clear()` 函数时间复杂度应该只跟元素的个数有关，而跟 `unordered_map` 的索引表的大小无关，但由于 `STL` 的一个小失误，导致 `unordered_map` 直接变成 `n^2_map`（`n方map`），可以说是一个小失误令无数 OIer 惨痛 `TLE`，所以我在此写下这篇文章引以为戒。
