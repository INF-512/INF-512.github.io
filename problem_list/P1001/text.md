...
轮廓线有 $m+1$ 条边，至多有 $\left\lfloor\frac{m+1}{2}\right\rfloor$ 对连通的边，再加上 $0$ 表示没有插头的边，所以每条边有 $\left\lfloor\frac{m+1}{2}\right\rfloor+1$ 种类型，需要用 $3$ 个二进制位表示，所以一条轮廓线需要用 $3\times (m+1)$ 个二进制位表示，把 $m=10$ 代入算出 $33$，正好用一个 `long long` 存储。
...
如果 $le,up$ 中恰好一个为 $0$ 另一个不为 $0$，那么只能在当前格子放路径，方向有两个选择：
